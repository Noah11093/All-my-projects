...
class SupervisedLearning:
    ...
    def modelling(self):
        # select the columns that are related to 'ACCLASS' and do not have too much unique value
    ...
    def evaluation(self):
        ...
        for i in range(len(best_model)):
            ...
            template = {
                f"accuracy_{models[i]}" : accuracy_score(y_test, y_test_pred),
                f"precision_{models[i]}" : precision_score(y_test, y_test_pred),
                f"recall_{models[i]}" : recall_score(y_test, y_test_pred),
                f"f1_score_{models[i]}" : f1_score(y_test, y_test_pred),
                f"confusion_matrix_{models[i]}" : confusion_matrix(y_test, y_test_pred),
            }
            self.result.append(template)
            
            # print(f"Training Accuracy of {mod}", mod.score(X_train_final, y_train))
            # print(f"Accuracy of {mod}", accuracy_score(y_test, y_test_pred))
            # print(f"Precison of {mod}", precision_score(y_test, y_test_pred))
            # print(f"Recall of {mod}", recall_score(y_test, y_test_pred))
            # print(f"F1 Score of {mod}", f1_score(y_test, y_test_pred))
            # print(f"Confusion Matrix of {mod}")
            ...
            # plt.savefig(f'ROC - {models[i]}.png')
